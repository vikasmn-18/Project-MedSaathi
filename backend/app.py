...
def download_pdf():
    try:
        ...
        language = data.get('language', 'English')
        english_summary = data.get('english_summary', None)

        if not summary:
            return jsonify({"error": "No summary provided"}), 400

        filename = f"medsaathi_{uuid.uuid4().hex[:8]}.pdf"
        filepath = os.path.join(tempfile.gettempdir(), filename)
        generate_pdf(summary, language, english_summary, filepath)

        return send_file(
            filepath,
            as_attachment=True,
            download_name=f"MedSaathi_Report_{language}.pdf",
